...
public class MetaAuthenticationFailureHandler implements AuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        if (exception instanceof AccountExpiredException) {
            ...
            apiResponse = ApiResponse.fail(ResultCode.USER_ERROR_A0205);

        } else if (exception instanceof BadCredentialsException) {
            // 密码错误
            log.info("[登录失败] - 用户[{}]密码错误", username);
            apiResponse = ApiResponse.fail(ResultCode.USER_ERROR_A0210);

        } else if (exception instanceof CredentialsExpiredException) {
            // 密码过期
            log.info("[登录失败] - 用户[{}]密码过期", username);
            apiResponse = ApiResponse.fail(ResultCode.USER_ERROR_A0212);

        } else if (exception instanceof DisabledException) {
            // 用户被禁用
            log.info("[登录失败] - 用户[{}]被禁用", username);
            apiResponse = ApiResponse.fail(ResultCode.USER_ERROR_A0202);

        } else if (exception instanceof LockedException) {
            // 用户被锁定
            log.info("[登录失败] - 用户[{}]被锁定", username);
            apiResponse = ApiResponse.fail(ResultCode.USER_ERROR_A0204);

        } else if (exception instanceof InternalAuthenticationServiceException) {
            // 内部错误
            log.error(String.format("[登录失败] - [%s]内部错误", username));
            apiResponse = ApiResponse.fail(ResultCode.USER_ERROR_A0200);
        ...
        ResponseUtil.responseWriter(response, "UTF-8", HttpStatus.UNAUTHORIZED.value(), apiResponse);
    }
}
